package com.webnation.begonerobotexters.widgets
...
class SlidingTabLayout @JvmOverloads constructor(
    ...
    defStyle: Int = 0
) : HorizontalScrollView(context, attrs, defStyle), KoinComponent {
    /**
    ...
    private var textColorSelected = Color.GRAY
    ...
    private fun createDefaultTabView(context: Context): TextView {
        ...
        val padding =
            (TAB_VIEW_PADDING_DIPS * resources.displayMetrics.density).toInt()
    ...
    private fun populateTabStrip() {
        ...
        val tabClickListener: OnClickListener =
            TabClickListener()
        for (i in 0 until adapter!!.count) {
            ...
            if (mTabViewLayoutId != 0) {
                ...
                tabTitleView =
                    tabView.findViewById<View>(mTabViewTextViewId) as TextView
            }
            if (tabView == null) {
    ...
    fun setTabSelected(position: Int) {
        for (i in 0 until mTabStrip.childCount) {
            ...
            textView.setTextColor(if (position == i) textColorSelected else textColor)
            textView.isSelected = position == i
        }
    }
    ...
    init {
        ...
        isFillViewport = true
        mTitleOffset =
            (TITLE_OFFSET_DIPS * resources.displayMetrics.density).toInt()
